...
namespace server_dotnet.Services
{
    public class EmployeeService
    {
        ...
        public async Task<bool> CreateEmployee(Employee employee)
        {
            var result =
                await _dbService.EditData(
                    "INSERT INTO public.employee (id, name, nik, address, jobPositionId) VALUES (@Id, @Name, @Nik, @Address, @JobPositionId)",
                    employee);
            return true;
        }
        ...
        public async Task<Employee> UpdateEmployee(Employee employee)
        {
            var updateEmployee =
                await _dbService.EditData(
                    "Update public.employee SET name=@Name, nik=@Nik, address=@Address, jobPositionId=@JobPositionId WHERE id=@Id",
                employee);
            return employee;
        }
